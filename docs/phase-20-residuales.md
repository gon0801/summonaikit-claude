...
| R6 | El test del descartable no discrimina el bump | 20.10 | Decisión: se deja sin fila — el poder discriminante se midió aparte en 20.10 (`grep -qx 'app v1.1.2'`) y el test débil vive en el lab descartable, no en este repo; re-evaluable si el gate vuelve a apoyarse en grep de versión |
...
| R17 | NO-MERGE grok no distingue sin-vínculo de stale | 20.13 | Decisión: candidata a 22.x sin número asignado — distinguir sin-vínculo de stale en el NO-MERGE de grok; re-evaluable cuando se abra la fila |
...
| R34 | Hook no parsea con bash 3.2 del sistema; instalador rehúsa en macOS | 20.25 | Decisión: candidata a 22.x — reescribir el heredoc o el guard del instalador toca `hooks/`+`tools/`, fuera del alcance skill/map/harness; requiere decisión de producto; re-evaluable cuando se autorice tocar `hooks/` |
