...
| Herramienta de delegación | `subagent_spawn`, con `tool_input = {command_id, objective, role}`. El rol viaja en `role`, el parámetro con el que Muse lanza al hijo. **Corregido en la 23.9:** en esta captura también llegó `subagent_type` porque el prompt lo pidió; en el vivo de la 23.6 el modelo mandó solo `role` y el hook que leía `subagent_type` no acreditó ningún rol | fixture 08; `docs/evidence/phase-23/23.9/` |
...
| ¿Doble registro, proyecto y usuario? | Con el workspace confiado (sin confianza el de proyecto no carga), el hook corre **dos veces y en paralelo**, aunque el comando sea idéntico: dos PIDs arrancan en el mismo microsegundo y el turno dura 2.6 s con hooks de 2 s | `doble-registro.txt` |
