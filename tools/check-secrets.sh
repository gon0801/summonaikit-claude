...
if [ -n "$GL_BIN" ]; then
  ...
  gl_args=(dir --no-banner --exit-code 1 --redact)
  if [ -f "$repo_dir/.gitleaks.toml" ]; then
    gl_args=(dir --config "$repo_dir/.gitleaks.toml" --no-banner --exit-code 1 --redact)
  fi
  if "$GL_BIN" "${gl_args[@]}" "${archivos[@]}" >/dev/null 2>&1; then
    exit 0
  fi
  echo "check-secrets: FAIL — gitleaks encontro secretos (ver 'gitleaks dir --redact' a mano para ubicarlos):" >&2
  printf '  %s\n' "${archivos[@]}" >&2
  exit 1
fi
...
for a in "${archivos[@]}"; do
  ...
  # -H fuerza el nombre de archivo aunque grep reciba uno solo; sed 's/:.*//'
  # deja "archivo:linea" sin el contenido: no filtrar lo que se viene a
  # impedir es parte del contrato de esta herramienta.
  mientras="$(grep -HnE "$re_par|$re_url|$re_fmt" -- "$a" 2>/dev/null | sed 's/:.*//')"
  if [ -n "$mientras" ]; then
