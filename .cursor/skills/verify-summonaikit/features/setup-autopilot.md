...
- `setup-lock-held` reports a LIVE held lock (exit 3), prints the recovery hint in executable form `bash tools/saikit-setup-autopilot.sh --liberar-lock`, and executes that hint innocuously against a COPY of the fixture (exit 0, the copy's lock is released, the original stays held).
