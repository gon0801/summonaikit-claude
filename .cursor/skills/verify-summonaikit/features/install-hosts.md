...
- `hosts-muse-install` installs `--host muse` against the fake Muse (`tests/fixtures/muse/muse-falso.sh` via `SAIKIT_MUSE_BIN`): five `saikit-harness-id 23.3` entries across the five events, four marked profiles without `model:`/`effort:`, and no hook copy under the muse config (it reuses the Claude dest).
- `hosts-quitar-muse-dry` reports and classifies the muse retirement without executing it: per piece with the `— se quitaria` anchor, one unknown agent classified `— no se quitaria`, the full muse config tree keeps content-and-tree identity, and the five entries plus four profiles stay present.
...
- Case `hosts-quitar-zcode-dry`: action Dry-run the zcode retirement after a zcode install, with one unknown agent and one foreign file planted; command `control-summonaikit drive install-hosts`; observable `dry-run: --quitar-zcode no ejecuta la retirada`, per-piece classification (`quitaria N entrada(s) 5.4`, each marked agent, `DESCONOCIDO … no se quitaria`), content-and-tree snapshot identity of `.zcode`, and config entries plus agent profiles still present.
- Case `hosts-quitar-grok-dry`: action Dry-run the grok retirement after a grok install, with one unknown agent and one foreign file planted; command `control-summonaikit drive install-hosts`; observable `dry-run: --quitar-grok no ejecuta la retirada`, JSON/hook/agents classified per piece with the `— se quitaria` anchor, content-and-tree snapshot identity of `.grok`, and hook+JSON+four profiles still present.
- Case `hosts-muse-install`: action Install `--host muse` with the fake Muse after creating its config dir (a missing dir means Muse is not installed, exit 2); command `control-summonaikit drive install-hosts`; observable five entries `saikit-harness-id 23.3` over `SessionStart UserPromptSubmit PreToolUse PostToolUse Stop`, four profiles with the ownership mark and no `model:`/`effort:`, and the Claude dest bytes unchanged with no muse hook copy.
- Case `hosts-quitar-muse-dry`: action Dry-run the muse retirement after a muse install, with one unknown agent and one foreign file planted; command `control-summonaikit drive install-hosts`; observable `dry-run: --quitar-muse no ejecuta la retirada`, entries/agents classified per piece with the `— se quitaria` anchor, content-and-tree snapshot identity of the muse config dir, and five entries plus four profiles still present.
...
- muse reuses the Claude hook copy too (no `--check --host muse` value); its install is validated with the fake Muse, never a live profile or model call.
