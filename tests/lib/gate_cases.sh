...
# 23.13 (fase 23, carril B) — el reporte DEVUELTO cita el token y no arma.
# El fixture es el payload real capturado del transcript de la sesion que
# midio el defecto (brief-23): hand-back agent-message de 19958 caracteres
# con 5 citas de -saikit en backticks. Con master arma (ROJO); con el skip
# estricto no deja estado.
caso_g1_reporte_devuelto_con_token_no_arma() {
  lab_limpiar_estado
  lab_run prompt claude "$(lab_fixture_reporte_devuelto)"
  if lab_hay_estado; then
    _mal "un reporte devuelto que cita el token no debe armar el gate — 23.13"
    return
  fi
}

# Reverso humano (PR #30, misma leccion que
# caso_g1_mencion_humana_de_la_marca_sigue_armando): un prompt HUMANO con
# -saikit que MENCIONA el marco del hand-back en medio del texto arma igual —
# la estricta exige el anuncio en primera linea, la laxa jamas saltea el armado.
caso_g1_mencion_humana_agent_message_sigue_armando() {
  lab_limpiar_estado
  lab_run prompt claude "$(lab_payload_prompt 'revisa el reporte devuelto -saikit: el marco <agent-message> trae el hand-back del revisor')"
  if ! lab_hay_estado; then
    _mal "un prompt humano con -saikit que MENCIONA el marco debe armar igual — 23.13"
    return
  fi
  _contiene "stdout" "$LAB_OUT" 'SUMMONAIKIT HARNESS REQUIRED'
}

# Reverso del desarme: un reporte devuelto que NO cita el token tampoco desarma
# un turno armado (la red laxa del hand-back). Sin la red, caia en el desarme
# A4-c2 y el Stop posterior dejaba salir el turno sin recibo.
caso_g1_reporte_devuelto_sin_token_no_desarma() {
  lab_sembrar 123456 1 1 1 "implementer,verifier,reviewer"
  lab_run prompt claude "$(lab_payload_prompt 'Another Claude session sent a message:\n<agent-message from=\"a838887c478f0fa98\">\n[Subagent hand-back] reporte limpio del revisor: sin hallazgos, sin token.\n</agent-message>')"
  if ! lab_hay_estado; then
    _mal "un reporte devuelto sin token no debe desarmar el turno armado — 23.13"
    return
  fi
  _igual "task_hash conservado" "$(sed -n 's/^task_hash=//p' "$LAB_ESTADO_PATH")" "123456"
}
...
# 23.16 (fase 23, carril B): `bash tests/run.sh ; echo` acredita aunque el
# runner falle y su salida no traiga senal de fracaso — el exit del comando es
# el del `echo` (0), no el del runner. Tras el arreglo, un `;` despues del
# runner tapa el credito, salvo `EXIT:0` textual en la salida (lo que
# `echo EXIT:$?` imprime cuando el runner salio 0) o encadenado con `&&`.
caso_g2_runner_punto_y_coma_tapa_exit_no_acredita() {
  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash 'bash tests/run.sh ; echo listo' 'listo')"
  _igual "runner propio con ; echo y salida limpia no acredita" "$(lab_estado verified)" "0"
...
# runner quedo documentado (`echo EXIT:$?` tras un runner que salio 0).
caso_g2_runner_punto_y_coma_con_exit_cero_si_acredita() {
  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash 'bash tests/run.sh ; echo EXIT:$?' 'EXIT:0')"
  _igual "runner con ; echo EXIT:$? y EXIT:0 si acredita" "$(lab_estado verified)" "1"
}

# 23.16 r2 (CodeRabbit PR #349, Major): la respuesta va ANTES y solo el
# comando trae la marca. `... ; grep EXIT:0 ...` con stderr limpio NO acredita
# aunque `tool_response` preceda a `tool_input`: la regla se aplica solo sobre
# el valor de la respuesta, nunca sobre lo que sigue en el documento. El
# control verde confirma que el acotamiento no tapa la evidencia real.
caso_g2_runner_respuesta_antes_marca_solo_en_comando_no_acredita() {
  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash_respuesta_antes 'bash tests/run.sh ; grep EXIT:0 notas.txt' 'sin coincidencias')"
  _igual "respuesta antes y EXIT:0 solo en el comando no acredita" "$(lab_estado verified)" "0"

  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash_respuesta_antes 'bash tests/run.sh ; echo EXIT:$?' 'EXIT:0')"
  _igual "respuesta antes y EXIT:0 en la respuesta si acredita" "$(lab_estado verified)" "1"
}

# 23.16 r2-revisor (codex, unica ronda sobre el arreglo): la respuesta
# trunca NO acredita aunque el fragmento traiga EXIT:0 (fail-closed: sin
# cierre no hay valor), y el CRLF no tapa la evidencia real con la respuesta
# antes. Las dos truncas darian credito con el lector viejo (`sed
# s/.*"tool_response"//`), que acreditaba el fragmento.
caso_g2_runner_respuesta_trunca_no_acredita() {
  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash_respuesta_trunca 'bash tests/run.sh ; echo listo' '{"stdout":"EXIT:0"')"
  _igual "respuesta objeto trunca con EXIT:0 no acredita" "$(lab_estado verified)" "0"

  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash_respuesta_trunca 'bash tests/run.sh ; echo listo' '"EXIT:0')"
  _igual "respuesta cadena trunca con EXIT:0 no acredita" "$(lab_estado verified)" "0"

  lab_sembrar 123456 0 0 0 ""
  lab_run tool claude "$(lab_payload_bash_respuesta_antes_crlf 'bash tests/run.sh ; echo EXIT:$?' 'EXIT:0')"
  _igual "respuesta antes con CRLF y EXIT:0 real si acredita" "$(lab_estado verified)" "1"
}
...
# shellcheck source=preflight_cases.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/preflight_cases.sh"

GATES="LAB G1 G2 G3 G4 G5 G6 G7 G8 G9"
...
caso_g1_muse_contrato_cabe_en_16k() {
  ...
  unset SAIKIT_RECETAS_DIR
  _contiene "claude conserva las reglas completas" "$LAB_OUT" 'Capability-first contract'
  _no_contiene "claude no recibe el contrato minimo" "$LAB_OUT" 'Contract shortened for Muse'
}
caso_g1_muse_contrato_tope_duro() {
  _recetas_lab
  _mt=0
  while [ "$_mt" -lt 150 ]; do
...
_muse_stop_cabe() { if [ "$(_muse_bytes)" -le "$1" ]; then echo si; else echo "no ($(_muse_bytes) bytes)"; fi; }
# Peor Stop sintetico: violacion adversary con autopilot=1 y carril full
# (los dos parrafos adosados) y una ruta de violacion larga. $1=cycle
# (0 = bloqueo, 2 = presupuesto agotado), $2=autopilot.
_muse_siembra_bloqueo() {
  mkdir -p "$(dirname "$LAB_ESTADO_PATH")" 2>/dev/null || true
  {
    printf 'task_hash=medicion\n'
    printf 'cycle=%s\n' "$1"
    printf 'implemented=0\n'
    printf 'verified=0\n'
    printf 'agents_seen=\n'
    printf 'lane=full\n'
    printf 'autopilot=%s\n' "$2"
    ...
    printf 'adv_paths=\n'
    printf 'adv_violation=1\n'
    printf 'adv_violation_paths=/fuera/muy/largo/nombre-de-archivo-violacion.ts\n'
  } > "$LAB_ESTADO_PATH"
}

# 23.11(e) — el Stop de muse no lleva tope: medido, el peor motivo
# (bloqueo adversary-early con autopilot=1, lane=full y ruta larga) pesa
# 2899 bytes y el presupuesto agotado 1996, contra los 16384 que corta Muse
# (margen 5.6x; el unico elemento dinamico es la lista de rutas, ya
# redactada). No se implementa el tope: cortaria el motivo que el modelo
# necesita leer, por un riesgo teorico. Este caso lo deja vigilado: si la
# salida del Stop alguna vez supera la mitad del tope (8192), hay que
# revisitar (e). Lo atrapa muse_stop_adosa_contrato (16221 bytes).
caso_g1_muse_stop_cabe_en_tope() {
  _mu_backup="$LAB_ESTADO_PATH"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  _muse_siembra_bloqueo 0 1
  lab_run stop muse "$(lab_payload_muse_fixture fixture-16-real2-delegacion-Stop.json)"
  _igual "bloqueo adversary-early cabe en 8192" "$(_muse_stop_cabe 8192)" "si"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  _muse_siembra_bloqueo 2 1
  lab_run stop muse "$(lab_payload_muse_fixture fixture-16-real2-delegacion-Stop.json)"
  _igual "presupuesto agotado cabe en 8192" "$(_muse_stop_cabe 8192)" "si"
  LAB_ESTADO_PATH="$_mu_backup"
}
...
# Task 23.9 — medido en el vivo 23.6 (2026-09-18): sin que el prompt lo pida,
# el modelo pasa el rol SOLO en `role`, el parametro de Muse. Con el hook que
# leia subagent_type, los tres roles corrieron y ninguno se acredito.
caso_g3_muse_spawn_solo_role_acredita() {
  _mu_backup="$LAB_ESTADO_PATH"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  lab_run tool muse "$(lab_payload_muse_derivado spawn-implementer-solo-role.json)"
  _contiene "role solo deja pendiente" "$(lab_estado muse_pending)" "id-implementer-001:implementer"
  lab_run tool muse "$(lab_payload_muse_derivado wait-implementer.json)"
  _igual "wait ready acredita el rol de role" "$(lab_estado agents_seen)" "implementer"
  LAB_ESTADO_PATH="$_mu_backup"
}
...
# 23.11(b) — un despacho del reviewer RECHAZADO no mueve last_review (el
# aceptado ya esta cubierto desde la 23.9): el aviso cuenta la revision
# cuando el reviewer termino, no al despacharlo ni al rechazarlo.
caso_g3_muse_rejected_no_marca_revision() {
  _mu_backup="$LAB_ESTADO_PATH"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-06.json)"
  _muse_apuntar_estado "01a0b27a-5bdc-7780-8d38-e6051a29b041"
  rn_file="$(dirname "$LAB_ESTADO_PATH")/harness-state-review-notice.env"
  lab_run tool muse "$(lab_payload_muse_derivado spawn-rejected-con-id.json)"
  _vacio "el despacho rechazado del reviewer no marca la revision" "$(grep '^last_review=.' "$rn_file" 2>/dev/null)"
  LAB_ESTADO_PATH="$_mu_backup"
}

# 23.11(a) — una sola lectura de tool_response por evento: la cache se
# asignaba dentro de una sustitucion de comando y no persistia, asi cada
# clave (status, subagent_id) re-leia el campo completo.
caso_g3_muse_tool_response_una_lectura() {
  _mu_backup="$LAB_ESTADO_PATH"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  _cuenta="$LAB/tr-cuenta.txt"
  export SAIKIT_CUENTA_TR="$_cuenta"
  : > "$_cuenta"
  lab_run tool muse "$(lab_payload_muse_fixture fixture-10-real2-delegacion-PostToolUse-subagent_spawn.json)"
  _igual "spawn lee tool_response una vez" "$(tr -d '[:space:]' < "$_cuenta" | wc -c | tr -d ' ')" "1"
  : > "$_cuenta"
  lab_run tool muse "$(lab_payload_muse_fixture fixture-15-real2-delegacion-PostToolUse-subagent_wait.json)"
  _igual "wait lee tool_response una vez" "$(tr -d '[:space:]' < "$_cuenta" | tr -d ' ' | wc -c | tr -d ' ')" "1"
  unset SAIKIT_CUENTA_TR
  LAB_ESTADO_PATH="$_mu_backup"
}

caso_g3_muse_rearm_no_hereda_pendiente() {
  _mu_backup="$LAB_ESTADO_PATH"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  lab_run tool muse "$(lab_payload_muse_fixture fixture-10-real2-delegacion-PostToolUse-subagent_spawn.json)"
  _contiene "precondicion spawn deja pendiente" "$(lab_estado muse_pending)" "01a0b27b-eb4e-7190-ba37-95245b9e48ef:reviewer"
  lab_run prompt muse "$(lab_payload_muse_derivado prompt-saikit-sesion-07.json)"
  _muse_apuntar_estado "01a0b27b-dc89-78e2-bc42-ef9808d62097"
  _vacio "re-arm borra muse_pending" "$(lab_estado muse_pending)"
