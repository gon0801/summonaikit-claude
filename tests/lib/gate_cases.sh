...
caso_g1_dsh_arma_y_aisla_estado() {
  ...
  _no_vacio "estado bajo state/dsh/ (D2: rama HOST=dsh)" "$_ruta_dsh"
...
# Phase 15 — la ceremonia bajo HOST=dsh (Stop claude-like, JSON). Turno completo
# (armado + implementer + verifier con evidencia real + reviewer) con un recibo
# COMPLETO => cierra con exit 0. dsh NO es ciego (15.1): el verifier deja traza de
# tool real (lab_payload_bash con runner), y el recibo es completo.
caso_g2_dsh_ceremonia_cierra() {
  ...
  lab_run stop dsh "$(lab_payload_stop "$_RECIBO_VERIF_SUBAGENTE_ACREDITA")"
  _igual "exit code" "$LAB_RC" "0"
  _no_contiene "motivo" "$LAB_OUT" 'Missing verification evidence'
  _no_contiene "motivo" "$LAB_OUT" 'Missing Retro gate summary'
}
