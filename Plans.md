...
| 1.4 | `[Test]` `[lane:fast]` `[tdd:skip:captura-manual-con-persona-adelante]` **Deuda declarada de la 1.2**: la línea base se grabó contra payloads *reconstruidos* — fieles a la forma real y con los campos sacados de transcripts reales, pero no una captura cruda de stdin. Claude Code fotografía los hooks al arrancar, así que la captura de verdad necesita una sesión nueva: un paso manual que no se podía hacer dentro del turno. `tools/capture-payloads.sh` ya tiene el procedimiento | Los payloads de las 3 fases se capturan con `--instalar`/`--cosechar` en un repo descartable y se comparan contra los fixtures; toda diferencia de forma se corrige en el fixture y la línea base se regraba; el diff de `tests/golden/baseline.txt` queda en el commit como declaración de qué cambió al usar payloads reales (si no cambia nada, eso también se declara) | 1.2 | cc:TODO |
