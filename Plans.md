...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 17.1 | `[Skill]` `[lane:gate]` `[tdd:required]` **`saikit-verificar-app`** (D11): skill a nivel usuario que genera `verify/` en el repo del usuario (`LEEME.md` con mapa de 3–5 funciones en español + Launch/Doctor/Drive/Evidence/Cleanup); **Drive = e2e en el framework que el repo ya tiene, invocado por un comando que `TEST_RUNNER_RE` acredita** (`node --test` se envuelve en `npm test`); framework nuevo solo con sí explícito del usuario, pinneado, nunca `npx @latest`; sin sí ⇒ Drive "manual, pendiente" y `verify_app: n/a`; se corre UNA vez antes de entregar | `tests/test_verificar_app.sh` contra 2 repos fixture (node con `node --test`, python+pytest) en sandbox: archivos esperados + el comando de Drive matchea `TEST_RUNNER_RE` LEÍDO del hook con `grep -o`; repo sin framework ⇒ propone y espera el sí, no instala | 16.1 | cc:TODO |
...
| 17.3 | `[Rastro]` `[lane:gate]` `[tdd:required]` **`.saikit/decisiones/<task>.tsv`** (D12): columnas `cuando|etapa|decision|por_que|evidencia|resultado`; `tools/saikit-decision.sh` append seguro con redacción (patrones del hook + `ghp_`/`github_pat_`/`gho_`/`sk-`/`AKIA`/`xox[bp]-`); contrato: `Close:` cita la ruta + sección **Atención**; **se commitea antes de despachar al reviewer**; adversary lee el tsv | `tests/test_saikit_decision.sh`: append crea/agrega sin partir filas; cada forma de token sale `[REDACTED]`; tsv malformado se reporta, no se repara; golden regrabada con diff auditado (2 de 4) | 16.4 | cc:TODO |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 18.1 | `[Diseño]` `[lane:gate]` `[tdd:skip:diseño-y-medición]` **Diseño medido + plan paso a paso** (`docs/phase-18-autopilot-plan.md`): formas reales de `gh pr checks` (concluido / pendiente / **sin checks**) y `gh run list --commit`, `gh pr merge --match-head-commit` **sin `--delete-branch`** y si pasa el runtime floor sin prompt, flujo de revert de un squash medido, las 3 preguntas del setup, esquemas del veredicto (D16) y de `autopilot.json` (D15); crea el repo descartable `gon0801/<nombre>` con topic `saikit-descartable` + marcador | Cada precondición de D18 con su forma observada o `unknown`; el descartable creado, nombrado y documentado | 17.6 | cc:TODO |
...
| 18.3 | `[Veredicto]` `[lane:gate]` `[tdd:required]` **`.saikit/veredictos/<sha>.json`** (D16): el líder commitea (incluido el rastro) ANTES de despachar al reviewer; el reviewer graba `sha = git rev-parse HEAD` + campos; esquema + test de contrato; `.gitignore` de `veredictos/` creado por el hook; contrato: `Close:` cita sha y ruta | `tests/test_veredicto_contract.sh`: válido / `sha` ≠ HEAD ⇒ inválido / campo faltante ⇒ inválido; golden regrabada con diff auditado (3 de 4) | 17.4, 18.1 | cc:TODO |
| 18.4 | `[Merge]` `[lane:gate]` `[tdd:required]` **`tools/saikit-merge.sh` fail-closed y acotado** (D18): repo del cwd, PR de la rama actual, `baseRefName == config.rama`, `headRefOid == sha`; config leída de `origin/<rama>`; cruce con el estado del hook (`reviewer` en `agents_seen`, `Write` del veredicto atribuido al reviewer, comando del blast en `harness-evidence.log` con éxito); `gh pr merge --squash --match-head-commit <sha>` sin `--delete-branch`; borrado remoto aparte; registra `merge_commit`; `--dry-run`; **modo `--revert-de <merge_commit>`** (D19, P1 de Greptile en PR #95): sin estado del hook, exige merge_commit propio + diff exactamente inverso por `patch-id` + ningún otro commit + CI verde + `--match-head-commit` | `tests/test_saikit_merge.sh` con `gh` falso y git real en sandbox: todo ok ⇒ merge; CI rojo / sin checks / pendiente / veredicto de otro sha / commits después del veredicto / `blast.nivel<4` / `verifier: FAIL` / config ausente o `unknown` / PR que toca `autopilot.json` / PR de otra rama base / repo distinto / autor ajeno / sin estado del hook ⇒ NO mergea y nombra la razón; merge ok + borrado remoto falla ⇒ reporta sin reintentar; `--revert-de`: inverso exacto ⇒ merge; diff distinto del inverso / commit extra / merge_commit ajeno ⇒ NO; nunca `--admin` | 18.3 | cc:TODO |
| 18.5 | `[Post-merge]` `[lane:gate]` `[tdd:required]` **`tools/saikit-postmerge.sh`** (D19), turno desarmado: run por `gh run list --commit <merge_commit>`; sin run ⇒ `unknown` con heartbeat acotado, timeout ⇒ `unknown` sin revert; `GET salud_url` solo http(s), sin redirects, `--max-time`, redactada; rojo ⇒ `git revert <merge_commit>` (squash) SOLO del merge propio, PR de revert mergeado con `saikit-merge.sh --revert-de` (sin estado del hook: inverso exacto + CI verde), una vez; si el revert no puede mergearse ⇒ avisa que la rama está roja y cómo revertir a mano; mensaje en español desde el `Close:` redactado (≤4096); `telegram-send` opcional | `tests/test_saikit_postmerge.sh`: verde ⇒ mensaje; rojo ⇒ revert mergeado; revert rojo ⇒ para y avisa; salud caída ⇒ revert; sin run aún ⇒ `unknown`; merge_commit ajeno ⇒ no revierte | 18.4 | cc:TODO |
| 18.6 | `[Sentinel+Contrato]` `[lane:gate]` `[tdd:required]` **`-saikit:autopilot`** ⇒ full + `autopilot=1` en estado (D15) y párrafo del contrato (D21: qué hará solo, qué nunca; merge antes del recibo; `Close:` con sha mergeado o razón de no-merge); **sin checks nuevos en el Stop** | Rojo medido; caso arma-autopilot / sufijo desconocido ⇒ full sin flag; mutación; golden regrabada con diff auditado (4 de 4) | 16.4, 18.4 | cc:TODO |
| 18.7 | `[Setup]` `[lane:gate]` `[tdd:required]` **`saikit-setup-autopilot`**: 3 preguntas en español ⇒ `.saikit/autopilot.json` (esquema + test; `merge_despliega` nace `unknown`); lock `.saikit/autopilot.lock` (D20) + `.gitignore` idempotente de `veredictos/` y del lock; el instalador planta la skill (Windows-bound) | `tests/test_autopilot_config.sh`: JSON válido/inválido; `unknown` por defecto; segundo autopilot en el mismo repo ⇒ espera o reporta, nunca dos merges | 18.1 | cc:TODO |
