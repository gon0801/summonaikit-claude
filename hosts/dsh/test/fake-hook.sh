...
# Si "$FAKE_HOOK_FULL_LOG" esta set, escribe el PAYLOAD COMPLETO (stdin) por
# llamada, para poder asertar session_id/agent_type/tool_input (Gap1 plugin.test.js).
set -u
in="$(cat)"
...
if [ -n "${FAKE_HOOK_FULL_LOG:-}" ]; then
  printf '%s\n' "$in" >> "$FAKE_HOOK_FULL_LOG"
fi
